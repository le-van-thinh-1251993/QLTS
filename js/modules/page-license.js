...
window.QLTSPageLicense.init = async function () {
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        if (actionBtn) {
            ...
            else if (action === 'edit-asset') { const item = assets.find(a => String(a.id) === String(id)); if (item) { updateDropdowns(); document.getElementById('modal_assetId').value = item.id; ['modal_assetName', 'modal_assetConfig', 'modal_assetLocation', 'modal_assetStatus', 'modal_assetNotes'].forEach(k => document.getElementById(k).value = item[k.replace('modal_asset', '').toLowerCase()] || ''); document.getElementById('modal_assetLocation').value = item.location || ''; document.getElementById('modal_assetPurchaseDate').value = item.purchase_date ? item.purchase_date.toString().substring(0, 10) : ''; document.getElementById('modal_assetCost').value = item.cost || 0; document.getElementById('modal_assetSalvage').value = item.salvage_value || 0; document.getElementById('modal_assetLife').value = item.useful_life_months || ''; document.getElementById('modal_assetDepMethod').value = item.depreciation_method || 'straight_line'; document.getElementById('modal_assetInvoiceNumber').value = item.invoice_number || ''; document.getElementById('modal_assetDisposedDate').value = item.disposed_date ? item.disposed_date.toString().substring(0, 10) : ''; document.getElementById('modal_assetDisposedReason').value = item.disposed_reason || ''; document.getElementById('assetDisposedFields').classList.toggle('hidden', item.status !== 'Disposed'); document.getElementById('modal_assetCategory').value = categories.find(c => c.name === item.category)?.id || ''; document.getElementById('modal_assetSupplier').value = item.supplier_id || ''; document.getElementById('modalTitle').textContent = 'Sửa tài sản'; initAllUserDropdowns(item.user); openModal('assetModal'); } }
