document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTableLicenses(data) {
        ...
        // ... (Giữ nguyên đoạn tính trang pageData) ...
        const start = (licenseCurrentPage - 1) * ITEMS_PER_PAGE;
        const pageData = data.slice(start, start + ITEMS_PER_PAGE);

        if (pageData.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" class="p-8 text-center text-slate-400">Không có dữ liệu.</td></tr>';
            renderPagination('licensePagination', 1, 0, ITEMS_PER_PAGE, 'licenses');
            return;
        }
        ...
        tbody.innerHTML = pageData.map(item => {
            ...
            const editBtn = `<button data-action="edit-license" data-id="${item.id}" class="${btnClasses} text-green-600 hover:bg-green-100"><i class="fa-solid fa-pen"></i></button>`;
            const deleteBtn = `<button data-action="delete-license" data-id="${item.id}" class="${btnClasses} text-red-600 hover:bg-red-100"><i class="fa-solid fa-trash"></i></button>`;

            return `<tr class="border-b hover:bg-slate-50" data-id="${item.id}">
                <td class="p-4 font-semibold text-slate-700">${item.key_type}</td>
                <td class="p-4 font-mono text-xs text-slate-500">${item.license_key || ''}</td>
                ...
                <td class="p-4 flex items-center gap-2">
                    ...
                    <div class="tooltip">${editBtn}<span class="tooltiptext">Sửa license</span></div>
                    <div class="tooltip">${deleteBtn}<span class="tooltiptext">Xóa license</span></div>
                </td>
            </tr>`;
        }).join('');
        ...
        tbody.innerHTML = pageData.map(u => {
            ...
            return `<tr class="border-b hover:bg-slate-50">
                ...
                <td class="p-4 flex gap-2">
                    <div class="tooltip"><button data-action="view-user-assets" data-id="${u.id}" class="${btnClasses} text-slate-500 hover:bg-slate-200 hover:text-blue-600"><i class="fa-solid fa-eye"></i></button><span class="tooltiptext">Xem chi tiết</span></div>
    ...
    // 7. LISTENERS & EVENTS
    // =================================================================
    ['searchInput', 'searchUserInput', 'filterDepartment', 'filterLicenseType'].forEach(id => document.getElementById(id)?.addEventListener(id.includes('filter') ? 'change' : 'input', () => id.includes('User') ? applyUserFilters() : (id.includes('License') ? applyLicenseFilters() : applyAssetFilters())));
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        // =================================================================
        // [MỚI] LOGIC CLICK VÀO HÀNG (ROW) ĐỂ MỞ CHI TIẾT
        // =================================================================
        ...
        if (assetRow && !t.closest('button') && !t.closest('input') && !t.closest('a')) {
            ...
            if (item) {
                document.getElementById('modal_assetId').value = item.id;
                ['modal_assetName', 'modal_assetConfig', 'modal_assetLocation', 'modal_assetStatus', 'modal_assetNotes'].forEach(k => {
                    document.getElementById(k).value = item[k.replace('modal_asset', '').toLowerCase()] || '';
                });
                updateDropdowns();
                const catId = categories.find(c => c.name === item.category)?.id || '';
                document.getElementById('modal_assetCategory').value = catId;
                document.getElementById('modalTitle').textContent = 'Chi tiết tài sản';
                initAllUserDropdowns(item.user);
        ...
        if (actionBtn) {
            ...
            else if (action === 'view-user-assets') {
                const uAssets = assets.filter(a => a.user_id === id);
                const uLicenses = licenses.filter(l => l.user_id === id);
                let content = '<div class="text-left space-y-3"><div><h4 class="font-bold text-blue-600 mb-1">Tài sản đang giữ:</h4>';
                if (uAssets.length > 0) { content += '<ul class="list-disc pl-5 text-sm text-slate-700">'; uAssets.forEach(a => content += `<li><b>${a.name}</b> <span class="text-gray-500">(${a.config || 'N/A'})</span></li>`); content += '</ul>'; } else content += '<p class="text-sm text-gray-400 italic">Không có tài sản</p>';
                content += '</div><div><h4 class="font-bold text-green-600 mb-1">License đang giữ:</h4>';
                if (uLicenses.length > 0) { content += '<ul class="list-disc pl-5 text-sm text-slate-700">'; uLicenses.forEach(l => content += `<li><b>${l.key_type}</b> <span class="text-gray-500">(${l.package_type || ''})</span></li>`); content += '</ul>'; } else content += '<p class="text-sm text-gray-400 italic">Không có license</p>';
                content += '</div></div>';
                showInfoModal(content, `Chi tiết: ${users.find(u => u.id === id)?.name}`);
            }
    ...
    function openTransferLicenseModal(id) {
        ...
        selectEl.choicesInstance = newChoices;
